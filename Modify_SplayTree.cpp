...
class Tree {
public:
    virtual void insert(const std::string& name, int id) = 0;
    virtual void remove(const std::string& name) = 0;
    virtual int search(const std::string& name) = 0;
...
private:
    ...
    void saveNodeToFile(Node* node, std::ofstream& outFile) {
        if (!node) return;
        saveNodeToFile(node->left, outFile);
        outFile << node->name << " " << node->id << "\n";
        saveNodeToFile(node->right, outFile);
    }

    void loadNodeFromFile(Node*& node, std::ifstream& inFile) {
        std::string line, name;
        int id;
        while (std::getline(inFile, line)) {
            std::istringstream iss(line);
            if (!(iss >> name >> id)) { break; }
            insert(name, id);
        }
    }
...
public:
    ...
    void insert(const std::string& name, int id) override {
        if (root == nullptr) {
    ...
    int search(const std::string& name) override {
        root = splay(root, name);
...
private:
    ...
    void displayMenu() {
        ...
        std::cout << "5. Зберегти дерево у файл\n";
        std::cout << "6. Завантажити дерево з файлу\n";
        std::cout << "7. Вихід\n";
    }

public:
    UserInterface(Tree* t) : tree(t) {}
        ...
        while (true) {
            ...
            switch (choice) {
                ...
                case 4:
                    ...
                    break;
                case 5:
                    std::cout << "Введіть назву файлу для збереження: ";
                    std::cin >> filename;
                    tree->saveToFile(filename);
                    std::cout << "Дерево збережено у файл.\n";
                    break;
                case 6:
                    std::cout << "Введіть назву файлу для завантаження: ";
                    std::cin >> filename;
                    tree->loadFromFile(filename);
                    std::cout << "Дерево завантажено з файлу.\n";
                    break;
                case 7:
                    std::cout << "Вихід...\n";
                    return;
